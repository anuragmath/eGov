...
import static org.egov.utils.FinancialConstants.MODULE_NAME_APPCONFIG;
...
public class CreateVoucher {
		...
		try {
			...
			for (final EgBilldetails egBilldetails : billDetailslist) {
				...
				for (final EgBillPayeedetails egBillPayeedetails : subLedgerlist) {
					...
					subledgertDetailMap.put(VoucherConstant.CREDITAMOUNT, egBillPayeedetails.getCreditAmount() == null
							? BigDecimal.ZERO : egBillPayeedetails.getCreditAmount());
	...
	protected void insertIntoVoucherHeader(final CVoucherHeader vh) throws ApplicationRuntimeException {
		...
		vh.setCreatedBy(userMngr.getUserById(Long.valueOf(ApplicationThreadLocals.getUserId())));
		if (LOGGER.isInfoEnabled())
			LOGGER.info("++++++++++++++++++" + vh.toString());
		voucherService.persist(vh);
	...
	public String getVoucherNumberFormatPrefix(String keyName) {
		try {
			keyName = "vouchernumberprefix_" + keyName;
			List<AppConfigValues> configValuesByModuleAndKey = appConfigValuesService
					.getConfigValuesByModuleAndKey(MODULE_NAME_APPCONFIG, keyName);

			return configValuesByModuleAndKey.get(0).getValue();
		} catch (Exception e) {
			throw new ApplicationRuntimeException("Error while looking for appconfig for " + keyName);
		}
	}
	...
	private String getVoucherNumberPrefix(final String type, String vsubtype) {
		...
		case JVGENERAL:
			voucherNumberPrefix = getVoucherNumberFormatPrefix(FinancialConstants.JOURNAL_VOUCHERNO_TYPE);
			break;
		case JOURNALVOUCHER:
			voucherNumberPrefix = getVoucherNumberFormatPrefix(FinancialConstants.JOURNAL_VOUCHERNO_TYPE);
			break;
		case CONTRA:
			voucherNumberPrefix = getVoucherNumberFormatPrefix(FinancialConstants.CONTRA_VOUCHERNO_TYPE);
			break;
		case RECEIPT:
			voucherNumberPrefix = getVoucherNumberFormatPrefix(FinancialConstants.RECEIPT_VOUCHERNO_TYPE);
			break;
		case PAYMENT:
			voucherNumberPrefix = getVoucherNumberFormatPrefix(FinancialConstants.PAYMENT_VOUCHERNO_TYPE);
			break;
		case PURCHASEJOURNAL:
			voucherNumberPrefix = getVoucherNumberFormatPrefix(FinancialConstants.PURCHBILL_VOUCHERNO_TYPE);
			break;
		case WORKS:
			voucherNumberPrefix = getVoucherNumberFormatPrefix(FinancialConstants.WORKSBILL_VOUCHERNO_TYPE);
			break;
		...
		case FIXEDASSETJOURNAL:
			voucherNumberPrefix = getVoucherNumberFormatPrefix(FinancialConstants.FIXEDASSET_VOUCHERNO_TYPE);
			break;
		case CONTINGENTJOURNAL:
			voucherNumberPrefix = getVoucherNumberFormatPrefix(FinancialConstants.CBILL_VOUCHERNO_TYPE);
			break;
		case PURCHASE:
			voucherNumberPrefix = getVoucherNumberFormatPrefix(FinancialConstants.PURCHBILL_VOUCHERNO_TYPE);
			break;
		...
		case EXPENSE:
			...
			break;
		case SALARYJOURNAL:
			voucherNumberPrefix = getVoucherNumberFormatPrefix(FinancialConstants.SALBILL_VOUCHERNO_TYPE);
			break;
		case SALARY:
			voucherNumberPrefix = getVoucherNumberFormatPrefix(FinancialConstants.SALBILL_VOUCHERNO_TYPE);
			break;
		case FIXEDASSET:
			...
			break;
		case PENSIONJOURNAL:
			voucherNumberPrefix = getVoucherNumberFormatPrefix(FinancialConstants.PENBILL_VOUCHERNO_TYPE);
			break;
		case PENSION:
			voucherNumberPrefix = getVoucherNumberFormatPrefix(FinancialConstants.PENBILL_VOUCHERNO_TYPE);
			break;
		case ADVANCE:
			voucherNumberPrefix = getVoucherNumberFormatPrefix(FinancialConstants.ADVANCEBILL_VOUCHERNO_TYPE);
			break;
