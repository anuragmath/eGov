...
public class PaymentService extends PersistenceService<Paymentheader, Long> {
		...
		try {
			...
			if (billregister.getEgBillregistermis().getEgDepartment() != null)
				headerdetails.put(VoucherConstant.DEPARTMENTCODE,
						billregister.getEgBillregistermis().getEgDepartment().getCode());
	...
	protected Assignment getWorkflowInitiator(final Paymentheader paymentheader) {
		Assignment wfInitiator = assignmentService
				.findByEmployeeAndGivenDate(paymentheader.getCreatedBy().getId(), new Date()).get(0);
		return wfInitiator;
	}
	...
	private void updateVoucherHeader(final Map<String, String[]> parameters, final CVoucherHeader existingVH,
		...
		final String vType = existingVH.getVoucherNumber().substring(0,
				Integer.parseInt(voucherService.getVoucherNumberFormatPrefix(FinancialConstants.VOUCHERNO_TYPE_LENGTH)));
		if (LOGGER.isDebugEnabled())
	...
	public List<ChequeAssignment> getPaymentVoucherForRTGSInstrument(final Map<String, String[]> parameters,
		...
		if (voucherHeader.getName() == null
			...
			query = getSession()
					...
					.addScalar("bankAccountId", BigDecimalType.INSTANCE).addScalar("billId", BigDecimalType.INSTANCE)
					.addScalar("billNumber").addScalar("expenditureType")
	...
	public List<ChequeAssignment> getDirectBankPaymentVoucherForRTGSInstrument(final Map<String, String[]> parameters,
		...
		if (voucherHeader.getName() == null
			...
			query = getSession()
					...
					.addScalar("departmentName").addScalar("voucherDate").addScalar("paidTo")
					.addScalar("paidAmount", BigDecimalType.INSTANCE).addScalar("chequeDate").addScalar("bankAccNumber")
					.addScalar("bankAccountId", BigDecimalType.INSTANCE).addScalar("expenditureType")
					.setResultTransformer(Transformers.aliasToBean(ChequeAssignment.class));
		...
		if (parameters.get("paymentMode")[0].equals(FinancialConstants.MODEOFPAYMENT_CHEQUE)) {
			...
			if (voucherHeader.getName() != null
				...
				query = getSession()
						...
						.addScalar("voucherid", BigDecimalType.INSTANCE).addScalar("voucherNumber")
						.addScalar("voucherDate").addScalar("paidAmount").addScalar("chequeDate").addScalar("paidTo")
						.setResultTransformer(Transformers.aliasToBean(ChequeAssignment.class));
				if (LOGGER.isDebugEnabled())
				...
				query = getSession().createSQLQuery(
						...
						.addScalar("voucherid", BigDecimalType.INSTANCE).addScalar("voucherNumber")
						.addScalar("voucherDate").addScalar("paidAmount", BigDecimalType.INSTANCE)
						.addScalar("chequeDate").setResultTransformer(Transformers.aliasToBean(ChequeAssignment.class));
				if (LOGGER.isDebugEnabled())
					LOGGER.debug(" for non salary and remittance" + query);
				chequeAssignmentList = query.list();
				// below one handles
	...
	public Paymentheader createPaymentHeader(final CVoucherHeader voucherHeader, final Integer bankaccountId,
		...
		paymentheader.setVoucherheader(voucherHeader);
		final Bankaccount bankaccount = getSession().load(Bankaccount.class, bankaccountId.longValue());
