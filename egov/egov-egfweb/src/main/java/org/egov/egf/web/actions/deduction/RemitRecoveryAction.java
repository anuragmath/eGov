...
public class RemitRecoveryAction extends BasePaymentAction {
	...
	private String generateMessage(Paymentheader paymentheader, WorkflowBean workflowBean) {
		...
		case WorkflowConstants.ACTION_REJECT:
			message = getText("payment.reject", new String[] { paymentheader.getVoucherheader().getVoucherNumber(),
					workflowBean.getApproverName(), workflowBean.getApproverDesignationName() });
			break;
		case WorkflowConstants.ACTION_FORWARD:
			...
			if (paymentheader.getVoucherheader().getVouchermis().getBudgetaryAppnumber() != null)
				message = message + getText("budget.appr.number.success",
						new String[] { paymentheader.getVoucherheader().getVouchermis().getBudgetaryAppnumber() });
			break;
		case WorkflowConstants.ACTION_CANCEL:
			message = getText("payment.cancel", new String[] { paymentheader.getVoucherheader().getVoucherNumber() });
			break;
	 ...
	 *
	 *
	 * 1.Creates one EgRemittance with paymentvoucher is set 2.updates every
	 * EgRemittanceGldtl for selected Bills 3.Creates EgRemittanceDetail per
	 * selected Bill
	 *
	 */
	...
	private void loadApproverUser(final String type) {
		...
		if (paymentheader != null && paymentheader.getPaymentAmount() != null) {
			if (LOGGER.isInfoEnabled())
				LOGGER.info(
						"paymentheader.getPaymentAmount() >>>>>>>>>>>>>>>>>>> :" + paymentheader.getPaymentAmount());
		...
		if (paymentheader != null && paymentheader.getVoucherheader().getFiscalPeriodId() != null)
			map = voucherService.getDesgByDeptAndTypeAndVoucherDate(atype, scriptName,
					paymentheader.getVoucherheader().getVoucherDate(), paymentheader);
		else
	...
	public String sendForApproval() {
		...
		addActionMessage(generateMessage(paymentheader, workflowBean));
		// paymentheader = paymentActionHelper.sendForApproval(paymentheader,
		// workflowBean);
	...
	/*
	 * not used private void updateMiscBillDetail() { Miscbilldetail
	 * miscbillDetail = (Miscbilldetail) persistenceService.find(
	 ...
	 * 
	 * public void setCutOffDate(String cutOffDate) { this.cutOffDate =
	 * cutOffDate; }
	 */
